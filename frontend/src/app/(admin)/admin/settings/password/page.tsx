...
export default function AdminChangePasswordPage() {
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await adminAxios.post('/change-password', {
        currentPassword,
        newPassword,
      });
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 max-w-2xl">
        <form onSubmit={handleChangePassword} className="space-y-6">
          ...
          <button
            ...
            className="w-full py-3 bg-green-600 hover:bg-green-700 disabled:bg-gray-400 text-white font-bold rounded-xl transition-colors disabled:cursor-not-allowed flex items-center justify-center gap-2"
          >
            {loading ? (
              <>
